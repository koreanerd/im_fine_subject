...
- [설계 개요](#설계-개요)
  ...
  - [2. 패턴별 비교 및 선택 근거](#2-패턴별-비교-및-선택-근가)
...
| **값 수정
(Edit, Table)** | 사용자가 테이블 셀에서 Value 수정 → Apply 버튼 클릭 → 수정된 값으로 `appState` 업데이트 → 모든 UI 리렌더링 |
| **값 삭제
(Delete, Table)** | 사용자가 테이블의 Delete 버튼 클릭 → 해당 항목을 `appState`에서 제거 → 모든 UI 리렌더링 |
| **JSON 편집 후
Apply** | 사용자가 JSON 에디터에서 데이터 수정 → Apply 버튼 클릭 → 수정된 JSON을 `appState`로 변환하여 반영 → 모든 UI 리렌더링 |
